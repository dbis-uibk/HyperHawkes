...
"""
recbole_custom.data.dataloader.abstract_dataloader
################################################
"""
...
from recbole_custom.utils import InputType, FeatureType, FeatureSource, ModelType
...
class AbstractDataLoader(torch.utils.data.DataLoader):
    ...
    :class:`~recbole_custom.data.interaction.Interaction` when it is iterated.
    ...
    def _neg_sampling(self, inter_feat):
        if self.neg_sample_args.get("dynamic", False):
            ...
            neg_item_feat = Interaction(
                {self.iid_field: neg_candidate_ids}
            )
