...
"""
recbole_custom.data.dataloader.general_dataloader
################################################
"""
...
from recbole_custom.data.dataloader.abstract_dataloader import (
    AbstractDataLoader,
    NegSampleDataLoader,
)
from recbole_custom.data.interaction import Interaction, cat_interactions
from recbole_custom.utils import InputType, ModelType
    ...
    def collate_fn(self, index):
        ...
        if (
            ...
            for idx, uid in enumerate(uid_list):
                ...
                transformed_data = self._dataset[index]#self.transform(self._dataset, self._dataset[index])
...
class FullSortEvalDataLoader(AbstractDataLoader):
    ...
    def collate_fn(self, index):
        ...
        if not self.is_sequential:
            ...
            transformed_interaction = interaction#self.transform(self._dataset, interaction)
