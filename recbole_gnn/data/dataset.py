...
class GeneralGraphDataset(RecBoleDataset):
    ...
    def get_bipartite_time_inter_mat(self, row='user', row_norm=True):
        ...
        edge_time = self.inter_feat[self.time_field]

        return edge_index, edge_weight, edge_time

    def get_bipartite_dgl_graph(self):
        user_id = self.inter_feat[self.uid_field]
        item_id = self.inter_feat[self.iid_field]
        time = self.inter_feat[self.time_field]

        graph_data = {('item', 'by', 'user'): (item_id, user_id),
                      ('user', 'pby', 'item'): (user_id, item_id)}
        graph = dgl.heterograph(graph_data)
        graph.edges['by'].data['time'] = time
        graph.edges['pby'].data['time'] = time
        graph.nodes['user'].data['user_id'] = torch.arange(1, self.user_num + 1)
        graph.nodes['item'].data['item_id'] = torch.arange(1, self.item_num + 1)
        return graph


class DynamicGraphDataset(GeneralGraphDataset):
    def __init__(self, config):
        super().__init__(config)

    def get_adj_list(self):
        src_l = self.inter_feat[self.uid_field]
        dst_l = self.inter_feat[self.iid_field]
        ts_l = self.inter_feat[self.time_field]
        e_idx_l = torch.arange(0, len(src_l))
    ...
    def build(self):
        ...
        full_adj_list = self.get_adj_list()
        for dataset in datasets:
            dataset.full_adj_list = full_adj_list
        return datasets
...
class HyperHawkesDataset(SequentialDataset):
    def __init__(self, config):
        super().__init__(config)
